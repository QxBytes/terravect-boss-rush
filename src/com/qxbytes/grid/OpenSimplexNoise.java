...
public class OpenSimplexNoise {
	...
	public static double[][] generateNoise(int WIDTH, int HEIGHT, double FEATURE_SIZE) {
		...
		for (int y = 0; y < HEIGHT; y++)
		{
			for (int x = 0; x < WIDTH; x++)
			{
				...
				values[x][y] = ((value + 1.0)/2);
				//System.out.println(values[x][y]);
			}
		}
		return values;
	}
	public static double[][] generateNoise(int WIDTH, int HEIGHT, double FEATURE_SIZE, int roomsoffx, int roomsoffy) {
		double[][] values = new double[WIDTH][HEIGHT];
		OpenSimplexNoise noise = new OpenSimplexNoise(seed);
		int xoffs = roomsoffx * Game.TILESX;
		int yoffs = roomsoffy * Game.TILESY;
		for (int y = 0; y < HEIGHT; y++)
		{
			for (int x = 0; x < WIDTH; x++)
			{
				double value = noise.eval((x+xoffs) / FEATURE_SIZE, (y+yoffs) / FEATURE_SIZE, 0.0);
				values[x][y] = ((value + 1.0)/2);
				//System.out.println(values[x][y]);
			}
		}
	...
	public static double generateNoise(int roomsoffx, int roomsoffy) {
		OpenSimplexNoise noise = new OpenSimplexNoise(seed);
		return ((noise.eval(roomsoffx / (Grid.FEATURE_SIZE/1.25),  roomsoffy / (Grid.FEATURE_SIZE/1.25)) + 1.0) / 2.0);
	}
